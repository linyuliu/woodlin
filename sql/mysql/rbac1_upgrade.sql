...
-- 【废弃通知】
-- 
-- 本脚本包含 MySQL 存储过程，不符合最佳实践。
-- RBAC1 的业务逻辑应在 Java 应用层实现，而非数据库层。
-- 
-- 请使用新的完整脚本：
-- 1. woodlin_complete_schema.sql - 包含所有表结构（含 RBAC1）
-- 2. woodlin_complete_data.sql   - 包含所有初始数据
-- 
-- 这两个脚本已整合了 RBAC1 功能，无需再单独执行升级脚本。
-- 
-- 如果你是新项目，请直接使用完整脚本。
-- 如果你需要升级现有数据库，请参考 README.md 中的升级指南。
-- 
-- =============================================

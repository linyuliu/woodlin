...
- `woodlin-web`: Vue 3.5 + TypeScript admin UI. Source under `src/` is split into `api/`, `assets/`, `components/` (global `W*` widgets + `RightToolbar` / `PermissionButton` / `ParentView`), `composables/`, `config/`, `constants/`, `directives/`, `layouts/` (`DefaultLayout` with sidebar/header/tabs/breadcrumb), `locales/` (vue-i18n zh-CN / en-US), `router/` (instance + dynamic routes + guards), `stores/modules/` (Pinia: `user`, `app`, `route`, `tabs`, `permission`, `dict`, `tenant`), `styles/`, `types/`, `utils/`, `views/`.
...
- Permission checks: use `v-permission` for权限点、`v-role` for角色码，按钮场景优先 `<PermissionButton permission="system:user:add">`. These checks read from `useUserStore()` and short-circuit for super admin (`*:*:*`) / admin role.
...
- `cd woodlin-web && npm install && npm run dev`: install dependencies and start frontend dev server.
